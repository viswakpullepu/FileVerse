...
export default function WordToPdf() {
  ...
  const convertToPdf = async () => {
    ...
    try {
      const response = await fetch('http://localhost:8000/api/convert/word-to-pdf', {
        method: 'POST',
        body: formData,
      });
  ...
  return (
    <div className="tool-page" style={{ maxWidth: '900px', margin: '0 auto', padding: '2rem' }}>
      ...
      <p style={{ marginTop: '1rem', color: '#666' }}>
        Convert your DOC and DOCX files securely into PDF format.
      </p>
      ...
      {!downloadUrl ? (
        <div style={{ marginTop: '2rem' }}>
          ...
          >
            <UploadCloud size={48} color="#315ea5" style={{ marginBottom: '1rem' }} />
            <p style={{ fontSize: '1.2rem', fontWeight: 'bold' }}>{file ? file.name : 'Select Word File (.docx)'}</p>
          ...
          {file && (
            <button 
              ...
              style={{ width: '100%', marginTop: '2rem', padding: '1rem', fontSize: '1.2rem', backgroundColor: '#315ea5', color: 'white', display: 'flex', justifyContent: 'center', alignItems: 'center', gap: '0.5rem' }}
            >
              <FileText size={20} />
        ...
        <div style={{ marginTop: '2rem', textAlign: 'center' }}>
          <div style={{ padding: '3rem', backgroundColor: '#f0fdf4', border: '1px solid #bbf7d0', borderRadius: '12px', marginBottom: '2rem' }}>
            ...
            <a 
              href={downloadUrl} 
              download={`converted_${file.name.replace(/\.docx?$/, '')}.pdf`} 
              className="btn"
