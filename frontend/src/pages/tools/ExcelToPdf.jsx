...
export default function ExcelToPdf() {
  ...
  const convertToPdf = async () => {
    ...
    try {
      const response = await fetch('http://localhost:8000/api/convert/excel-to-pdf', {
        method: 'POST',
        body: formData,
      });
  ...
  return (
    <div className="tool-page" style={{ maxWidth: '900px', margin: '0 auto', padding: '2rem' }}>
      ...
      <p style={{ marginTop: '1rem', color: '#666' }}>
        Convert your spreadsheets (.xls, .xlsx) securely into PDF format.
      </p>
      ...
      {!downloadUrl ? (
        <div style={{ marginTop: '2rem' }}>
          ...
          >
            <UploadCloud size={48} color="#2a7433" style={{ marginBottom: '1rem' }} />
            <p style={{ fontSize: '1.2rem', fontWeight: 'bold' }}>{file ? file.name : 'Select Excel File'}</p>
            <span style={{ fontSize: '0.9rem', color: '#888', display: 'block', marginTop: '0.5rem' }}>
              {file ? 'Click to change file' : 'or drop file here (.xlsx, .xls)'}
            </span>
            <input 
              id="file-upload" 
              type="file" 
          ...
          {file && (
            <button 
              ...
              style={{ width: '100%', marginTop: '2rem', padding: '1rem', fontSize: '1.2rem', backgroundColor: '#2a7433', color: 'white', display: 'flex', justifyContent: 'center', alignItems: 'center', gap: '0.5rem' }}
            >
              <FileText size={20} />
        ...
        <div style={{ marginTop: '2rem', textAlign: 'center' }}>
          <div style={{ padding: '3rem', backgroundColor: '#f0fdf4', border: '1px solid #bbf7d0', borderRadius: '12px', marginBottom: '2rem' }}>
            ...
            <a 
              href={downloadUrl} 
              download={`converted_${file.name.replace(/\.xlsx?$/, '')}.pdf`} 
              className="btn"
